...
**Status:** v1 (derived from the codebase at v1.0.2, `README.md`, and `temp/README_private.md`)
...
| Level | Meaning |
|---|---|
| **MUST / MUST NOT** | A rule the source documents or code state explicitly as a constraint. Breaking it requires an explicit, recorded decision (see [Amendment](#12-amendment)). |
...
Flashcard Library is a mobile-first flashcard study app. A user loads or creates decks of question/answer pairs and studies them with an animated flip card.
...
Over roughly a year the app generalized from that scenario into a general-purpose study tool. It hit MVP in mid-2025, shipped as a PWA, and is now in the "add features and polish for app store release" phase.
...
- All user data lives on the device. There are no accounts, no sync service, no analytics, and no telemetry.
...
- The author's personal identity is deliberately screened behind a business entity. Changes **MUST NOT** surface the author's personal contact details, address, or real-name attribution in shipped artifacts — app metadata, store listings, bundle identifiers, build config, exported files, or the public repo. Publish under the business entity and the established pseudonymous channels only.

### 2.2 Mobile-first — MUST
...
### 2.3 Zero-friction start — SHOULD
...
### 2.4 The user's data is the user's — MUST
...
- Any increase to `MAX_DECKS` / `MAX_QUESTIONS` is re-validated for memory on a low-end Android device before shipping — that is what these numbers were set from.
...
*Descriptive:* additional modes are planned — full-random (card is not popped), single-pass with a disabled "next" at the end, quiz/multiple-choice, audio/car mode, show-all-at-once. The architecture anticipates this: `QuizScreen.js` has a hardcoded `whichScreen = "review"` switch waiting for siblings to `ReviewScreen`. Adding modes is expected; they should not compromise grab-bag as the default.

### 5.2 Flip card — MUST

- A card shows one side at a time and flips on tap; the direction (Q→A or A→Q) is user-selectable mid-session.
...
- **TXT:** alternating lines — question on odd lines, answer on even lines. Blank lines are dropped. An odd trailing line is discarded rather than paired with nothing.
- **CSV:** two columns, `question,answer`. Parsed by the project's own parser (`src/components/Deck/parseCsv.js`), not by a third-party library at runtime.
- **Manual entry:** the in-app "create deck" textarea uses the same alternating-line format as TXT, so there is one mental model for the user.
...
Export produces a plain-text file named `export_YYYYMMDD_HHMMSS.txt`, decks most-recent-first, each preceded by a `===` / `=== Deck name: X` header. Native builds write to cache and hand off to the OS share sheet (user chooses the destination); web builds trigger a Blob download. The web branch exists because PWAs have no native filesystem — **MUST** keep both paths working when touching Export.
...
- **Routing:** expo-router, stack-based, all screens `headerShown: false` — the app renders its own `Toolbar` instead. **MUST** keep real navigation for the Android hardware back button; it must never exit the app from a sub-screen.
...
- **Modals:** presented as callouts from the toolbar/menu (`AppMenuModal`, `HelpModal`, `FileHelpModal`, `TutorialModal`, `InstallModal`, `ConfirmModal`). Modal styles were deliberately consolidated; new modals SHOULD reuse the existing components rather than adding another style family. **MUST:** on Android, the hardware back button dismisses an open modal (with the unsaved-data warning where applicable) instead of navigating.
...
## 10. Platforms and build

**MUST:**

- Minimum platform versions: **iOS 17**, **Android API 32** (`minSdkVersion: 32`, `deploymentTarget: "17.0"`). These were set deliberately; raising or lowering them is a product decision, not a build tweak.
- The `use-latest-callback: ^0.2.3` override in `package.json` **MUST NOT** be removed. Without it the FAB component throws `_useLatestCallback.default is not a function`.
- The web build ships as an installable, offline-capable PWA: single-output Expo web, `standalone` display, service worker generated via `workbox-config.js`, hosted under the `/flashcard_library` base path.

*Descriptive:* React Native 0.81 / Expo ~54 / React 19, expo-router 6, EAS Build for iOS and Android (dev, preview, production profiles; `appVersionSource: "remote"`, production auto-increments). iOS beta distribution goes through TestFlight.
...
**MUST** before a release build:

- No `@todo` markers left in shipped source.
- Verify on a real Android device and a real iPhone, not just web. Several past bugs (CSV import, keyboard covering inputs, appbar inconsistency, modal dismissal) were device-only.
...
Not commitments. Recorded so that today's structure is understood as a starting point rather than a finished shape.

**Study modes:** quiz mode (multiple choice, distractors drawn from the same deck); audio / car mode (question, pause, answer — with a noted risk around Bluetooth); language mode driving TTS voice selection; full-random continuous mode; single-pass mode with a hard end; show-all-at-once; persisted mode selection; per-mode settings.
...
**UI:** tablet / multi-column layout (with teacher feedback on a Figma sketch first); swipe to advance; animated and swipeable help pages; custom card themes; dark-mode elevation via white overlay instead of shadows.

**Business:** app store release under the business entity; a tip option for free users after a usage threshold.
